...
function find_intervalA_ls(h :: LineModel,
    ...
    # Before starting the algorithm we vcheck to see if a step of 1.0 satisfies
    # both the Wolfe and the Armijo condition
    ti=1.0
    ht = obj(h,ti)
    gt = grad!(h, ti, g)
    ...
    # We will compute the value of the funtion and it's derivative at the current
    # point when/if necessary. NaN is there for the first verboseLS
    φti=NaN
    dφti=NaN

    admissible, tired = stop_ls(stp_ls, dφti, iter; kwargs...)

    verboseLS && @printf("iter tim1        dφtim1        φtim1         ti
                          dφti        φti\n")
    verboseLS && @printf("%4d %7.2e %7.2e  %7.2e  %7.2e  %7.2e  %7.2e \n",
                         iter, tim1, dφtim1, φtim1, ti, dφti, φti)

    while !(admissible | tired)
      # Implementation of the 3.5 Linesearch ALgorithm as presented by
      # Nocedal & Wright
      φti = φ(ti)
