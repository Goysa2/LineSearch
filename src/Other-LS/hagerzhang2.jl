...
function _hagerzhang2!{T}(df::AbstractNLPModel,
    ...
    if mayterminate &&
          satisfies_wolfe2(c, phic, dphic, phi0, dphi0, philim, delta, sigma)
        if display & LINESEARCH > 0
    ...
    while iter < linesearchmax
        ...
        iswolfe, iA, iB = secant22!(df, x, s, xtmp, lsr, ia, ib, philim, delta, sigma, display)
        if iswolfe
...
function secant22!{T}(df::AbstractNLPModel,
    ...
    push!(lsr, c, phic, dphic)
    ic = length(lsr)
    if satisfies_wolfe2(c, phic, dphic, phi0, dphi0, philim, delta, sigma)
        if display & SECANT2 > 0
